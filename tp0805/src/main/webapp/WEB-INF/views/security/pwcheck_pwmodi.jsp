	...
	$.ajax({
		...
		success: function(json){
			...
			if(result.code == "success"){
				window.location.replace("/member/change_pw_view");
			} else {
				alert(result.desc);
			}
		}
					...
					<div class="text-center">
						<form id="info_frm">
							<c:if test="${uinfo.mid != null}">
								<p class="test2-1">비밀번호 변경을 위해 기존 비밀번호를 입력해주세요.</p>
							...
							<button type="button" class="btn btn-info" onclick="pw_check()">입력</button>
