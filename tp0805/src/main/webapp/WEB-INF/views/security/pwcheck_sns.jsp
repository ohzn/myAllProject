...
<script>
	...
	function join() {
		window.location.replace("/homejoin/joincheck");
	}
	...
	$.ajax({
		...
		success: function(json){
			...
			if(result.code == "success"){
				window.location.replace("/member/userinfo?mid="+id);
			} else {
				alert(result.desc);
			}
		}
            ...
            <div class="col-sm-7">
				...
				<div class="justify-content-center">
					<div class="text-center">
						<form id="sns_check">
							<p class="test2-1">개인정보 조회를 위해 계정을 확인해주세요.</p>
							<c:choose>
								
								<c:when test="${user.flatform == 'google'}">
									<input type="hidden" id="mid" name="mid" value="${user.name}"> <br/>
									<button type="button" class="btn btn-primary btn-block mb-4" onclick="sns_check()">Google로 계정확인하기</button>
								</c:when>
								<c:when test="${user.flatform == 'kakao'}">
									<input type="hidden" id="mid" name="mid" value="${user.name}"> <br/>
									<button type="button" class="btn btn-warning btn-block mb-4" onclick="sns_check()">Kakao로 계정확인하기</button>
								</c:when>
								<c:when test="${user.flatform == 'naver'}">
									<input type="hidden" id="mid" name="mid" value="${user.name}"> <br/>
									<button type="button" class="btn btn-success btn-block mb-4" onclick="sns_check()">Naver로 계정확인하기</button>
								</c:when>
							</c:choose>
						</form>
					</div>
