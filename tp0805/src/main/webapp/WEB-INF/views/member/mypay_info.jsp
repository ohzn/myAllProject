...
<!-- End Header -->
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-7">
	            <h2 style="font-size: 18pt; margin-top: 5%;"> 주문 상세 보기
	            </h2> 
	            ...
	            <div style="text-align:center;">
	            	...
	            	발송 상태 : 
	            	<c:choose>
						<c:when test="${pay.pStatus == 0}"> 
							<span style="color:red;">준비중</span>
						</c:when>
						<c:when test="${pay.pStatus == 1}"> 
							<span style="color:blue; font-style:bold;">발송됨</span>
						</c:when>
						<c:when test="${pay.pStatus == 2}"> 
							기타
						</c:when>
					</c:choose>
				</div>
	            	...
	            	<c:choose>
						<c:when test="${order != null}">  
							<c:forEach items="${order}" var="odto" varStatus="st">
								<div class="row">
									<div class="col-sm-2" style='margin: auto; text-align:left;'>
										<img width="150px;" src="/upload/${odto.cPhoto1}" /><br/>
									</div>
									<div class="col-sm-6" style='margin: auto; text-align:left;'>
										<a href="goods_view?cId=${odto.cId}">${odto.cName}</a><br/>
										${odto.cPrice} 원<br/>
										${odto.pCount} 개<br/>
									</div>
									<div class="col-sm-2" style='margin: auto; text-align:center;'>
										${fn:substring(odto.pDate, 0, 10)}<br/>
									</div>
								</div>
					...
					<br/>
					수령인 : ${pay.adrName}<br/>
					배송지 : (${pay.aZipcode}) ${pay.aMain}<br/>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ${pay.aSub}<br/>
					<c:if test="${pay.pDeliverymemo != null}">
						배송 메모 : ${pay.pDeliverymemo}<br/>
	                </c:if>
