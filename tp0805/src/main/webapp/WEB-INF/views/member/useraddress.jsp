...
<!-- End Header -->
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-7">
	            <h2 style="font-size: 18pt; margin-top: 5%;"> 배송지 관리
	            </h2> 
						...
						<c:choose>
							<c:when test="${adr != null}">
								<c:forEach items="${adr}" var="dto" varStatus="st">
									...
									배송지명 : ${dto.aName} <br/>
									수령자 이름 : ${dto.adrName} <br/>
									우편번호 : ${dto.aZipcode} <br/>
									주소 : ${dto.aMain} <br/>
									상세주소 : ${dto.aSub} <br/>
									<br/>
									<button type=button onclick="del_address('${dto.mId}' ,'${dto.aName}')">배송지 삭제</button>
									<hr/>
								</c:forEach>
							</c:when>
							<c:otherwise>
