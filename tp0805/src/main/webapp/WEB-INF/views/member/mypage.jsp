...
<script>
	...
	function userAddress() {
		window.location.replace("/member/getList");  // 다른곳도 수정해야함
	}
	// 공지사항
				...
				<hr style="margin-bottom: 30px;" />
					<div class="row">
						...
						<hr style="width: 20%; height: 2px; color: black;">
							...
							<c:choose>
								<c:when test="${dinfo.mid != null}">
								<c:forEach items="${dinfo}" var="dto" varStatus="status"> 
									<td><img width="100px;" src="/upload/${dto.cPhoto1}" /></td>
									<td><a href="goods_view?cId=${dto.cId}">${dto.cName}</a></td>
						...
						<hr style="width: 20%; height: 2px; color: black;">
							<c:choose>
								<c:when test="${wish.mid != null}">
									<img width="100px;" src="/upload/${wish.cPhoto1}" /><br/>
									<br/>
									<a href="goods_view?cId=${wish.cId}">${wish.cName}</a><br/>
									<br/>
								</c:when>
								<c:otherwise>
									찜한 상품이 없습니다.
									<a href="/goods/goods_list">상품 보러가기</a><br/>
								</c:otherwise>
							</c:choose>
						</div>
					</div>
