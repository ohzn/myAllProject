...
<meta charset="UTF-8">
    ...
    <title>고객센터 : KIDULT</title>

	<script src="http://code.jquery.com/jquery.js"></script>

<script>
function notice_delete(bId) {
	let = confirm("글을 삭제하시겠습니까?");
	if (let == true) {
		notice_delete_ajax(bId);
    } else { }
}

function notice_delete_ajax(bId) {
	$.ajax({
...
<!-- End Header -->
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-7">
	            <h2 style="font-size: 18pt; margin-top: 5%;"> 공지사항 
	                <button class="btn btn-light btn-sm btn-block" style="float: right;" onclick="notice_write()">
	                    글 쓰기
	                </button>
	            </h2> 
	            <hr style="margin-bottom: 0;">
	            <ul style="list-style: none; padding-left: 0; margin-bottom: 50%" class="accordion accordion-flush" id="accordionFlushExample">
	            
					<c:forEach items="${list}" var="dto">
					  <li class="accordion-item">
					    <h2 class="accordion-header" id="flush-headingOne">
					      <div class="accordion-button collapsed" data-bs-toggle="collapse" 
					      	   data-bs-target="#n${dto.bId}" 
					      	   aria-controls="${dto.bId}">
					        ...
					        <div style="float: left;
									    ...
									    letter-spacing: -.03em;
									    display: inline;
									    vertical-align: middle;"> ${dto.bTitle} </div> 
					        <div style="float: right;
					        			width: 15%;
									    font-size: 13px;
									    color: #999;
									    vertical-align: middle;
									    display: inline;"> ${fn:substring(dto.bDate, 0, 10)} </div>
					      </div>
						</h2>
						<div id="n${dto.bId}" 
						...
						<c:if test="${uinfo.authority == 'ROLE_ADMIN'}"> 
						  	<a type="button" class="btn btn-sm"
						  			...
						  			font-size: 8px; 
						  			margin-right: 1%; 
						  			margin-top: 1%;" onclick="notice_delete(${dto.bId})">삭제</a> &nbsp;&nbsp;
						  	<a type="button" class="btn btn-sm"
						  			style="float: right; 
						  			font-size: 8px; 
						  			margin-right: 1%; 
						  			margin-top: 1%;" href="/admin/notice_modify_view?bId=${dto.bId}">수정</a> &nbsp;
						</c:if>
	...
	<nav aria-label="Page navigation example">
		<ul class="pagination justify-content-center">
            ...
            <c:otherwise>
            	<li class="page-item">
                	<a class="page-link" href="notice_view?page=1" aria-label="Previous">
                  		<span aria-hidden="true">&laquo;</span>
                	</a>
            	</li>
			</c:otherwise>
			...
			<c:otherwise>
				<li class="page-item">
            		<a class="page-link" href="notice_view?page=${page.curPage - 1}">Previous</a>
            	</li>
			</c:otherwise>
			</c:choose>
            ...
            <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
				...
				<c:when test="${page.curPage == fEach}">
					...
					<c:otherwise>
            <li class="page-item">
            	<a class="page-link" href="notice_view?&page=${fEach}">${fEach}</a>
            </li>
            </c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:otherwise>
				<li class="page-item">
                	<a class="page-link" href="notice_view?page=${page.curPage + 1}">Next</a>
            	</li>
			</c:otherwise>
			</c:choose>
			...
			<c:otherwise>
				<li class="page-item">
                	<a class="page-link" href="notice_view?page=${page.totalPage}" aria-label="Next">
                  	<span aria-hidden="true">&raquo;</span>
                	</a>
            	</li>
			</c:otherwise>
