...
<script>
function go_delivery(pid) {
	$.ajax({
		url: '/admin/order_status_one',
		type: 'POST',
		data: { pid : pid },
		dataType: 'text',
		success: function(json) {
...
<!-- End Header -->
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-7">
	            <h2 style="font-size: 18pt; margin-top: 5%;"> 주문내역 관리
	            </h2> 
	            <hr style="margin-bottom: 20px;">
	            	<div class="row">
	            	<form action="/admin/order_status" method="get">
						<div class="col test2">
							<input type="submit" name="gocheck" value="발송처리" class="btn btn-success btn-sm mb-4 float-right" />
							<table class="table table table-hover">
								...
								<tbody>
									...
									<tr>
										<td><input type="checkbox" name="pids" value="${dto.pId}" /></td>
										<td>${fn:substring(dto.pDate, 0, 10)}</td>
										<td><a href="/admin/pay_info?pId=${dto.pId}">${dto.pId}</a></td>
										<td>${dto.totalPrice} 원</td>
										<td>
											<c:choose>
												<c:when test="${dto.pStatus == 0}"> 
													준비중
												</c:when>
												<c:when test="${dto.pStatus == 1}"> 
													발송됨
												</c:when>
												<c:when test="${dto.pStatus == 2}"> 
													기타
												</c:when>
											</c:choose>
										</td>
										<td>
											<c:if test="${dto.pStatus == 0}"> 
	                							<button type="button" class="btn btn-success btn-sm mb-4" onclick="go_delivery('${dto.pId}')">발송</button>
	                						</c:if>
										</td>
									</tr>
									</c:forEach>
					...
					<div class="row">
						...
						<nav aria-label="Page navigation example">
							<ul class="pagination justify-content-center">
					            ...
					            <c:otherwise>
					            	<li class="page-item">
					                	<a class="page-link" href="/admin/pay_list?page=1" aria-label="Previous">
					                  		<span aria-hidden="true">&laquo;</span>
					                	</a>
					            	</li>
								</c:otherwise>
								...
								<c:otherwise>
									<li class="page-item">
					            		<a class="page-link" href="/admin/pay_list?page=${page.curPage - 1}">Previous</a>
					            	</li>
								</c:otherwise>
								</c:choose>
					            ...
					            <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
									...
									<c:when test="${page.curPage == fEach}">
										...
										<c:otherwise>
					            <li class="page-item">
					            	<a class="page-link" href="/admin/pay_list?&page=${fEach}">${fEach}</a>
					            </li>
					            </c:otherwise>
									</c:choose>
								</c:forEach>
								...
								<c:otherwise>
									<li class="page-item">
					                	<a class="page-link" href="/admin/pay_list?page=${page.curPage + 1}">Next</a>
					            	</li>
								</c:otherwise>
								</c:choose>
								...
								<c:otherwise>
									<li class="page-item">
					                	<a class="page-link" href="/admin/pay_list?page=${page.totalPage}" aria-label="Next">
					                  	<span aria-hidden="true">&raquo;</span>
					                	</a>
					            	</li>
								</c:otherwise>
