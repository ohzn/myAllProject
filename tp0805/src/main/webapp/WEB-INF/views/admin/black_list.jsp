...
<!-- End Header -->
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-sm-7">
	            <h2 style="font-size: 18pt; margin-top: 5%;"> 블랙리스트 관리
	            </h2> 
						...
						<div class="col test2">
							<table class="table table table-hover">
								<thead class="thead-light">		
									<tr>
										<th scope="col">ID</th>
										<th scope="col">제재 유형</th>
										<th scope="col">제재 날짜</th>
										<th scope="col">제재 사유</th>
										<th scope="col">관리</th>
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										<td><a href="/admin/userview?mid=${dto.mid}">${dto.mid}</a></td>
										<td>
											<c:choose>
												<c:when test="${dto.btype == 1}"> 
													로그인 정지
												</c:when>
												<c:when test="${dto.btype == 2}"> 
													결제 정지
												</c:when>
											</c:choose>
										</td>
										<td>${fn:substring(dto.btime, 0, 10)}</td>
										<td>${dto.bmemo}</td>
										<td><button type="button" class="btn btn-success btn-sm mb-4" onclick="del_black('${dto.mid}')">해제</button></td>
									</tr>
									</c:forEach>
								</tbody>
							</table>
					...
					<div class="row">
						...
						<nav aria-label="Page navigation example">
							<ul class="pagination justify-content-center">
					            ...
					            <c:otherwise>
					            	<li class="page-item">
					                	<a class="page-link" href="/admin/blacklist?page=1" aria-label="Previous">
					                  		<span aria-hidden="true">&laquo;</span>
					                	</a>
					            	</li>
								</c:otherwise>
								...
								<c:otherwise>
									<li class="page-item">
					            		<a class="page-link" href="/admin/blacklist?page=${page.curPage - 1}">Previous</a>
					            	</li>
								</c:otherwise>
								</c:choose>
					            ...
					            <c:forEach var="fEach" begin="${page.startPage}" end="${page.endPage}" step="1">
									...
									<c:when test="${page.curPage == fEach}">
										...
										<c:otherwise>
					            <li class="page-item">
					            	<a class="page-link" href="/admin/blacklist?&page=${fEach}">${fEach}</a>
					            </li>
					            </c:otherwise>
									</c:choose>
								</c:forEach>
								...
								<c:otherwise>
									<li class="page-item">
					                	<a class="page-link" href="/admin/blacklist?page=${page.curPage + 1}">Next</a>
					            	</li>
								</c:otherwise>
								</c:choose>
								...
								<c:otherwise>
									<li class="page-item">
					                	<a class="page-link" href="/admin/blacklist?page=${page.totalPage}" aria-label="Next">
					                  	<span aria-hidden="true">&raquo;</span>
					                	</a>
					            	</li>
								</c:otherwise>
